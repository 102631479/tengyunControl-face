<template>
  <div class="flex">
    <div style="width: 420px;">
      <Form :model="loginForm" ref="form" :rules="ruleInline" @keydown.enter.native="submit">
        <div>
        ...
        <div>
          <Button type="primary" long size="large" class="lh-30" @click="submit" :loading="loading">登录</Button>
        </div>
      </Form>
      ...
      <div class="flex border-box" style="margin-top:180px;">
        ...
        >立即注册</div>
        <div class="flex-sub f-size-14 cup t-center pt-14 color-333 pb-14 m-6" @click="$router.push({ name: 'setPwd' })">忘记密码</div>
      </div>
      ...
      await login(this.loginForm)
        .then((d) => {
          ...
          if (process.env.NODE_ENV === "development") {
            Cookie.set("token", d.data.token, { domain: ".zhihuiwenlvyun.com", path: "", expires: 7 });
          } else {
